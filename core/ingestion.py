"""Kafka ingestion entry points for streaming and bulk write modes."""
...
def bulk_ingest(spark, max_messages=None):
    """Drain buffered Kafka records and write them in deterministic table batches."""
        ...
        try:
            while empty_polls < 3 and (max_messages is None or total_rows_written < max_messages):
                ...
                for table_name, table_rows in rows_by_table.items():
                    ...
                    projections = [
                        F.col(column_name)
                        if dtype in ("date", "timestamp") or column_name == "ingestion_timestamp"
                        else F.col(column_name).cast("string").alias(column_name)
                        for column_name, dtype in df_table.dtypes
                    ]
...
def streaming_ingest(spark):
    """Start the Structured Streaming query that feeds ``process_batch``."""
    with log_step("Start streaming ingestion"):
        ...
        try:
            ...
            flattened = parsed.select(
                ...
                "data_format",
                "ingestion_timestamp",
                explode_outer(col("rows")).alias("row"),
            )
            return (
