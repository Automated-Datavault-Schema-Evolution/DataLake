"""Pure helpers that shape row collections into deterministic write batches."""
...
def group_rows_by_table(entries: Iterable[tuple[str, Mapping[str, Any] | None]]) -> dict[str, list[dict[str, Any]]]:
    """Group row payloads by logical table name."""
...
def materialized_columns(rows: Iterable[Mapping[str, Any]], excluded: Iterable[str] = TECHNICAL_COLUMNS) -> list[str]:
    """Return the sorted list of materialized business columns in a batch."""
    ...
    return sorted(
        {
            ...
            for row in rows
            for key in row.keys()
            if key is not None and key not in excluded_set
        }
    )
...
def normalize_rows(rows: Iterable[Mapping[str, Any]], column_names: Iterable[str]) -> list[dict[str, Any]]:
    """Return rows with a stable column order and empty strings normalized to ``None``."""
    ordered_columns = [str(name) for name in column_names]
    return [
        {column: (None if row.get(column) == "" else row.get(column)) for column in ordered_columns}
        for row in rows
    ]
...
def date_like_columns(column_names: Iterable[str]) -> list[str]:
    """Return columns whose names imply date semantics."""
    return [
        column
        for column in column_names
        if column.lower().endswith("date") or column.lower() in {"dob", "dateofbirth"}
    ]
